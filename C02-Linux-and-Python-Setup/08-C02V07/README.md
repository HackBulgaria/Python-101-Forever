# C02V07 - Terminal commands: Part 3

## Materials

* [Video](https://youtu.be/yY5iYsufebI)
* [Slides](https://docs.google.com/presentation/d/1GBXE3oFUQj-PH3q2YfwfYaMBuI9D7tZQtlA7XxVLPBU/edit?usp=sharing)

## Code examples

* [stdin.py](stdin.py)
* [stdout.py](stdout.py)
* [stdout_print.py](stdout_print.py)
* [stdout_stderr.py](stdout_stderr.py)
* [combined.py](combined.py)

## Additional materials

* <https://en.wikipedia.org/wiki/Standard_streams>
* <https://en.wikipedia.org/wiki/End-of-file>
* <https://en.wikibooks.org/wiki/Python_Programming/Input_and_Output>
* <https://stackoverflow.com/questions/3263672/the-difference-between-sys-stdout-write-and-print>
